...
\title{\pkg{activelearning}: An \proglang{R} Package for Optimally Labeling Unlabeled Data for Classification with Active Learning}
...
\end{align}
where $\widehat{y} = \text{arg max}_y P_{\theta}(\widehat{y} | \bm x)$ is the observation that maximizes the \emph{a posteriori} probability under model $\theta$. Hence, the least confidence observation that is queried is
