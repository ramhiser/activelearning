...
#'
#' We require a user-specified supervised classifier and its corresponding prediction (classification) function. These must be specified as strings in 'cl_train' and 'cl_predict', respectively. We assume that the 'cl_train' function accepts two arguments, x and y, as the matrix of feature vectors and their corresponding labels, respectively. The 'cl_predict' function is assumed to accept a trained object as its first argument and a matrix of test observations as its second argument. Furthermore, we assume that 'cl_predict' returns a list that contains a 'posterior' component that is a matrix of the posterior probabilities of class membership; the (i,j)th entry of the matrix must be the posterior probability of the ith observation belong to class j. Usually, it is straightforward to implement a wrapper function so that 'cl_train' and 'cl_predict' can be used. Additional arguments to 'cl_train' and 'cl_predict' can be passed via '...'
#' 
#' Unlabeled observations in 'y' are assumed to have NA for a label.
#'
#' It is often convenient to query unlabeled observations in batch. By default, we query the unlabeled observation with the largest uncertainty measure value. With the 'num_query' the user can specify the number of observations to return in batch. If there are ties in the uncertainty measure values, they are broken by the order in which the unlabeled observations are given.
#'
