...
#'
#' The 'query by committee' approach to active learning uitilizes a committee of C classifiers
#' that are each trained on the labeled training data. Our goal is to "query the oracle" with
#' the observations that have the maximum disagreement among the C trained classifiers.
#'
#' Note that this approach is similar to "Query by Bagging" (QBB), but each committee member
#' is specified by the user. With the QBB approach, only one supervised classifier is
#' specified by the user, and each committee member is trained on a resampled subset of 
#' the labeled training data. Also, note that we we have implemented QBB as query_by_bagging.
#'
...
#'
#' In specifying the committee members, we require a list (called 'committee' in the arguments) with elements
#' corresponding to each supervised classifier (each committee member). Each component in the list 'committee'
...
#'
#' We require that each training function (specified in 'train') accept x and y as the matrix of observations
#' and their labels of class membership, respectively. A function wrapper can be used to satisfy our requirement.
#' The 'train_args' is a named list that contains the arguments that will be passed to the function specified
#' in 'train'. Lastly, the 'predict' function should accept the trained object from 'train' as its first argument
#' and a matrix of unlabeled test observations as its second argument. Furthermore, we assume that the 'predict'
#' function returns a list that contains a 'posterior' component that is a matrix of the posterior
...
#'
#' We provide an example here that uses the linear discriminant analysis (LDA) implementation in the MASS
#' package as well as the regularized discriminant analysis (RDA) implementation in the klaR package. Each
#' training function arguments 'x' for the data matrix and 'grouping' as the vector of class labels.
#' Furthermore, the RDA classifier accepts two optional tuning parameters, lambda and gamma. If the models
#' are not provided they are estimated automatically. In our example, we consider using both the RDA
#' model with and without user-specified tuning parameters. Note that both the LDA and RDA classifiers
#' use 'predict' as their classification functions. The specified 'committee' can be formulated by:
#'
...
#'
#' This method uses the 'foreach' package and is set to do the train each committee member
#' in parallel if a parallel backend is registered. If there is no parallel backend registered,
#' a warning is thrown, but everything will work just fine.
#'
